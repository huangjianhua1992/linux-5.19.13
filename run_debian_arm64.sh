...
export INSTALL_PATH=$LROOT/rootfs_virt_arm64/boot/
export INSTALL_MOD_PATH=$LROOT/rootfs_virt_arm64/
export INSTALL_HDR_PATH=$LROOT/rootfs_virt_arm64/usr/

kernel_build=$PWD/rootfs_virt_arm64/usr/src/linux/
rootfs_path=$PWD/rootfs_virt_arm64
rootfs_image=$PWD/rootfs_virt_arm64.ext4
...
make_kernel_menuconfig(){
		echo "start build kernel menuconfig..."
		make menuconfig
}
...
prepare_rootfs(){
		if [ ! -d $rootfs_path ]; then
			...
			## split -d -b 80m rootfs_virt_arm64.tar.gz -- rootfs_virt_arm64.part 
			#cat rootfs_virt_arm64.part0* > rootfs_virt_arm64.tar.gz
			tar -xvf rootfs_virt_arm64.tar.gz
		fi
}
...
build_kernel_devel(){
	...
	rm rootfs_virt_arm64/lib/modules/$kernver/build
	...
	ln -s /usr/src/linux rootfs_virt_arm64/lib/modules/$kernver/build

	# ln to debian linux-5.0/scripts
	ln -s /usr/src/linux-kbuild/scripts rootfs_virt_arm64/usr/src/linux/scripts
	#ln -s /usr/src/linux-kbuild/tools rootfs_virt_arm64/usr/src/linux/tools
}
...
update_rootfs(){
		if [ ! -f $rootfs_image ]; then
			...
			mkdir -p $rootfs_path
			echo "mount ext4 image into rootfs_virt_arm64"
